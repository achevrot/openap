...
class Drag(object):
    ...
    def get_dp(self, alt):
        ...
        if alt < isa.H_TROP:
            # bada (2.2-18)
            dP = (
                p_0 * (isa.temperature(Q_(alt, "m")) / t_0)**(-isa.G_0 / (isa.BETA_T * isa.R)) / p_0
            )
        else:
            # bada (2.2-20)
    ...
    def get_cl(self, mass, mach, dP, s, bank_angle=0):

        cl = (2 * mass * isa.G_0) / (dP * isa.P_0 * isa.GAMMA * s * mach*mach * cos(bank_angle))
        return cl
    
    @default_units(mass='kg', tas='kts', alt="m", bank_angle='radian')
    def clean(self, mass, tas, alt, bank_angle=Q_(0, 'radian')):
        ...
        cl = self.get_cl(mass, mach, dP, s, bank_angle)
